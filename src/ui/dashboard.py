...
class DashboardApp:
    def __init__(self, root, data_queue):
        ...
        # self.root.attributes('-fullscreen', True) # Uncomment for production
    ...
    def create_widgets(self):
        ...
        # Score Section (Right - Big Circle simulation)
        ...
        # Bottom Bar (RPM, Throttle, Engine Load)
    ...
    def process_data(self, data):
        # Ensure we have state tracking mapping
        ...
        # Updates Colors based on Labels only if changed
...
def data_worker(data_queue):
    ...
    # Initialize Scorer and Recorder
    ...
    while True:
        try:
            if adapter:
                # Get data (async for BLE, sync for Mock)
                if USE_MOCK:
                    raw_data = adapter.get_data()
                else:
                    # For BLE, need to use async
